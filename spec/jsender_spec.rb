...
describe Jsender do
  ...
  context 'ability to mixin Jsender into a class' do
    before :each do
      @some_class = TestClass.new
    end

    it 'includes a success method' do
      expect(@some_class.sample_success).to eq ({"status"=>"success", "data"=>{"result"=>nil, "notifications"=>["happy day"]}})
    end

    it 'includes a success method' do
      expect(@some_class.sample_error).to eq ({"status"=>"error", "message"=>"something went wrong"} )
    end

    it 'includes a success method' do
      expect(@some_class.sample_failure).to eq ({"status"=>"fail", "data"=>{"result"=>nil, "notifications"=>["a failure occurred"]}})
    end
  end
